...
// Removed FiSettings as it was unused
...
// Defined a specific type for RecentActivity instead of any
type RecentActivity = {
  id: string;
  user_id: string;
...
export default function Dashboard() {
  ...
  // Using the defined type for recentActivities
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('id, user_id, description, created_at') // Explicitly selecting fields
        .eq('user_id', user.id)
        ...
        if (error) {
            ...
            // Type casting data to RecentActivity[] as we selected the correct fields
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              ...
              <div className="relative">
                <div className="overflow-hidden">
                  <motion.div
                    key={contentTools[activeToolIndex].id} // Using tool id as key is better practice
              ...
              <div className="flex justify-center mt-6 space-x-2">
                {/* Using _ for unused first parameter (the item) */}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Changed index to _ and used tool.id as key */}
                {contentTools.slice(0, 4).map((tool) => (
                  <Link href={tool.path} key={tool.id}> {/* Using tool.id as key */}
                    <motion.div
            ...
            >
              ...
              <div className="mb-4">
                <div className="w-full bg-gray-800/50 rounded-full h-2.5 mb-2">
                  {/* Assuming max credits for the progress bar is 800 based on the label */}
                  <div className="bg-gradient-to-r from-yellow-400 to-yellow-500 h-2.5 rounded-full" style={{ width: `${Math.min(100, ((profile?.credits || 0) / 800) * 100)}%` }}></div> {/* Adjusted calculation for progress bar max */}
                </div>
                <div className="flex justify-between text-xs text-gray-400">
                  <span>0</span>
                  <span>800</span> {/* Hardcoded max credits */}
                </div>
              </div>
            ...
            >
              ...
              {recentActivities.length === 0 ? (
                ...
                <div className="space-y-3">
                  {recentActivities.map((activity, idx) => ( // idx is used for delay
                    <motion.div
                      key={activity.id || idx}
